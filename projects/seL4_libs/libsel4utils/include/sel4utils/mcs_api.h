...
static inline seL4_Error api_tcb_configure(seL4_CPtr tcb, seL4_CPtr ep, UNUSED seL4_CPtr timeout_ep,
                                           ...
                                           seL4_Word ipc_buffer_addr, seL4_CPtr ipc_buffer_cap, uint64_t world_id)
{
    printf("[DEBUG] api_tcb_configure called\n");
    printf("[DEBUG] world_id = %lu\n", world_id);
#ifdef CONFIG_KERNEL_MCS
    ...
    return seL4_TCB_Configure(tcb, ep, cspace, cdata, vspace, vdata,
                              ipc_buffer_addr, ipc_buffer_cap, world_id);
#endif
}
